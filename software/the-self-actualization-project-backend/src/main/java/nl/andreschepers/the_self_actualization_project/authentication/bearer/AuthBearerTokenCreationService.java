...
package nl.andreschepers.the_self_actualization_project.authentication.bearer;
...
import nl.andreschepers.the_self_actualization_project.authentication.bearer.exception.BearerVerifyJwtException;
...
public class AuthBearerTokenCreationService {
  ...
  private static final int REFRESH_TOKEN_EXPIRATION_IN_MILLI_SECONDS = 24 * 60 * 60 * 1000;
  ...
  public void init() throws KeyLengthException {
    // Create HMAC signer
    jwsSigner =
        new MACSigner(Base64.getDecoder().decode(authenticationConfigProperties.jwtSecret()));
  }
  ...
  private String createToken(JWTClaimsSet claimSet, Date expirationTime, String tokenType) {
  ...
  private String createToken(
    ...
    var jwtClaimsSet =
        builder.claim(TOKEN_TYPE_CLAIM_NAME, tokenType).expirationTime(expirationTime).build();
  ...
  public SignedJWT verifyHmacSignedTokenJwt(String token) {
    try {
      ...
      JWSVerifier verifier = new MACVerifier(Base64.getDecoder().decode(authenticationConfigProperties.jwtSecret()));
      if (!signedJWT.verify(verifier)) {
        throw new BearerVerifyJwtException(BEARER_JWT_VERIFICATION_NOT_VALID_ERROR_MESSAGE);
      }
      return signedJWT;
    } catch (ParseException | JOSEException e) {
      log.error("Failed to parse refresh token JWT for verification: [{}]", e.getMessage(), e);
